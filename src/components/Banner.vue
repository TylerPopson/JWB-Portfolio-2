...
class Pentagon {
    ...
    constructor(stopPoint: number) {
        ...
        this.path = ref(
            "M" +
                this.points.p1x.value +
                " " +
                this.points.p1y.value +
                " L" +
                this.points.p2x.value +
                " " +
                this.points.p2y.value +
                " L" +
                this.points.p3x.value +
                " " +
                this.points.p3y.value +
                " L" +
                this.points.p4x.value +
                " " +
                this.points.p4y.value +
                " L" +
                this.points.p5x.value +
                " " +
                this.points.p5y.value +
                " Z",
        );
    }
    ...
    animate = () => {
        ...
        if (this.delta > this.interval) {
            ...
            this.path.value =
                ...
                " L" +
                (this.points.p2x.value +=
                    this.animationSpeed / this.easingRate) +
                " " +
                this.points.p2y.value +
                " L" +
                (this.points.p3x.value +=
...
<template>
    <div
        id="home"
        class="flex w-screen h-screen -z-10 overflow-hidden bg-banner-image bg-fixed"
    >
        <div class="static flex flex-auto w-full h-full">
            ...
            <div
                class="absolute top-0 left-0 flex flex-col w-full md:w-1/2 h-full items-center justify-center"
                :style="{ opacity: contentTransparency }"
            >
                <div
                    class="flex w-5/6 md:w-1/2 aspect-square rounded-full bg-main-headshot-image bg-center bg-cover"
                ></div>
                <h3 class="text-5xl text-white font-overpassBold">
                    Jordan Burger
                </h3>
                <h5 class="text-3xl text-white font-overpassRegular">
                    Audio Engineer
                </h5>
            </div>
        </div>
